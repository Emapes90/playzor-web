...
export default function FAQ() {
  ...
  return (
    <section
      id="faq"
      aria-label="Frequently Asked Questions about Playzor Minecraft Hosting"
      className="py-16 sm:py-24 bg-black border-t border-white/5"
    >
      <div className="max-w-3xl mx-auto px-4 sm:px-6">
        <div className="text-center mb-8 sm:mb-16">
          ...
          <p className="text-zinc-400 text-sm sm:text-base">
            Everything you need to know about Playzor game server hosting.
          </p>
        </div>
        ...
        <div className="space-y-3 sm:space-y-4">
          {faqs.map((faq, index) => (
            ...
            >
              ...
              >
                <span
                  className={`font-bold text-sm sm:text-lg ${openIndex === index ? "text-white" : "text-zinc-300"}`}
                >
                  {faq.question}
                </span>
                <div
                  className={`p-1.5 sm:p-2 rounded-full shrink-0 ${openIndex === index ? "bg-white text-black" : "bg-white/5 text-white"}`}
                >
                  {openIndex === index ? (
